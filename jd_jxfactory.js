...
function assistFriend(sharepin) {
  return new Promise(async resolve => {
    ...
    const options = {
      'url': `https://raw.githubusercontent.com/hajiuhajiu/jdsign1112/master/backUp/total/jxfactory.json`,
      'headers': {
        "Accept": "*/*",
    ...
    $.get(options, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} 打工分享码请求失败，请检查网路重试`)
        } else {
          if (safeGet(data)) {
            data = JSON.parse(data);
            // if (data['ret'] === 0) {
            //   console.log(`帮好友打工：${sharepin}成功`)
            // } else {
            //   console.log(`帮好友打工[${sharepin}]失败：${data.msg}`)
            // }
          }
        }
      } catch (e) {
