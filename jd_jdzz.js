        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success) {
              ...
              if (data.resultCode === '9200008') console.log('不能助力自己团')
              else if (data.resultCode === '9200011') console.log('团已经助力过')
              else if (data.resultCode === '2400205') console.log('团已满')
              else if (data.resultCode === '2400203') {console.log('团助力次数已耗尽');$.canHelp = false}
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `https://gitee.com/xr2021/share/raw/master/JDZZCode`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
